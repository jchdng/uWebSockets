...
static  void write_callback(WebSocket webSocket, void *userData, bool cancelled) {
	WebSocket::PreparedMessage *preparedMessage = (WebSocket::PreparedMessage *)userData;
...
void WebSocket::sendPrepared(WebSocket::PreparedMessage *preparedMessage)
{
    ...
    write(preparedMessage->buffer, preparedMessage->length, false, write_callback, preparedMessage, true);
}
...
void WebSocket::handleFragment(const char *fragment, size_t length, OpCode opCode, bool fin, size_t remainingBytes, bool compressed)
{
    ...
    if (opCode < 3) {
        ...
        if (!remainingBytes && fin) {
            if (opCode == CLOSE) {
                boost::tuple<unsigned short, char *, size_t> closeFrame = Parser::parseCloseFrame(socketData->controlBuffer);
                close(false, boost::get<0>(closeFrame), boost::get<1>(closeFrame), boost::get<2>(closeFrame));
...
WebSocket::Address WebSocket::getAddress()
{
    ...
    if (addr.ss_family == AF_INET) {
		...
		WebSocket::Address wa = { ntohs(ipv4->sin_port), buf, "IPv4" };
        return wa;
    } else {
        sockaddr_in6 *ipv6 = (sockaddr_in6 *) &addr;
        inet_ntop(AF_INET6, &ipv6->sin6_addr, buf, sizeof(buf));
		WebSocket::Address wa = { ntohs(ipv6->sin6_port), buf, "IPv6" };
		return wa;
    }
}
...
void WebSocket::onReadable(uv_poll_t *p, int status, int events)
{
    ...
    if (received == SOCKET_ERROR || received == 0) {
        ...
        if (socketData->state == CLOSING && socketData->controlBuffer.length()) {
            boost::tuple<unsigned short, char *, size_t> closeFrame = Parser::parseCloseFrame(socketData->controlBuffer);
            if (!boost::get<0>(closeFrame)) {
				boost::get<0>(closeFrame) = 1006;
            }
            WebSocket(p).close(true, boost::get<0>(closeFrame), boost::get<1>(closeFrame), boost::get<2>(closeFrame));
        } else {
            WebSocket(p).close(true, 1006);
        }
        return;
...
void WebSocket::write_callback_onclose(WebSocket webSocket, void *data, bool cancelled) {
	if (!cancelled) {
		uv_os_sock_t fd;
...
void WebSocket::uvtimercb(uv_timer_t* handle) {
	WebSocket((uv_poll_t *)handle->data).close(true, 1006);
}
...
void WebSocket::close(bool force, unsigned short code, char *data, size_t length)
{
    ...
    if (force) {
        ...
        uv_poll_stop(p);
        uv_close((uv_handle_t *) p, uvclosecb<uv_poll_t>);
        ...
        if (socketData->prev) {
            ...
            uv_close((uv_handle_t *) socketData->prev, uvclosecb<uv_timer_t>);
        }
        ...
        uv_timer_start((uv_timer_t *) socketData->prev,uvtimercb, 15000, 0);
        ...
        write(sendBuffer, formatMessage(sendBuffer, &sendBuffer[length + 2], length, CLOSE, length, false), false, write_callback_onclose);
    }
}
