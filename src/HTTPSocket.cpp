...
struct Request {
    ...
    Request &operator++(int)
    {
        ...
        if (cursor[0] == '\r' && cursor[1] == '\n') {
            key = value = std::make_pair((char*)0,0);
        } else {
            for (; cursor[length] != ':' && cursor[length] != '\r'; length++);
            key = /*{cursor, length}*/std::make_pair(cursor,length);
            if (cursor[length] != '\r') {
                cursor += length;
                length = 0;
                while (isspace(*(++cursor)));
                for (; cursor[length] != '\r'; length++);
				value = /*{cursor, length}*/std::make_pair(cursor, length);
            } else {
                value = /*{0, 0}*/std::make_pair((char*)0, 0);
            }
        }
        return *this;
    }
...
uv_os_sock_t HTTPSocket::stop()
{
    ...
    uv_poll_stop(p);
	uv_close((uv_handle_t *)p, uvclosecb<uv_handle_t>);

    uv_timer_stop(t);
    uv_close((uv_handle_t *) t, uvclosecb<uv_timer_t>);
...
void HTTPSocket::onReadable(uv_poll_t *p, int status, int events)
{
    ...
    if (httpData->headerBuffer.find("\r\n\r\n") != std::string::npos) {
        ...
        std::pair<char *, size_t> secKey = std::make_pair((char*)0,0), extensions = std::make_pair((char*)0, 0);
