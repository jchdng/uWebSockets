...
struct PerMessageDeflate {
	...
	PerMessageDeflate(ExtensionsParser &extensionsParser, int options, std::string &response)
		: serverNoContextTakeover(false), clientNoContextTakeover(false)
    {
		memset(&readStream, 0, sizeof(z_stream));
		memset(&writeStream, 0, sizeof(z_stream));
