...
static void uvclosecbnull(uv_handle_t* handle) {
}

void Server::closeHandler(Server *server)
{
    if (!server->master) {
        uv_close((uv_handle_t *) &server->upgradeAsync, uvclosecbnull);
        uv_close((uv_handle_t *) &server->closeAsync, uvclosecbnull);
    }
    ...
    if (server->listenPoll) {
        ...
        uv_close((uv_handle_t *) server->listenPoll, uvclosecb<uv_poll_t>/*[](uv_handle_t *handle) {
...
void Server::uvasynccb_handleclose(uv_async_t* handle) {
	closeHandler((Server *)handle->data);
}

void Server::uvasynccb_upgradehandler(uv_async_t* handle)
{
	upgradeHandler((Server *)handle->data);
}

Server::Server(EventSystem &es, int port, unsigned int options, unsigned int maxPayload, SSLContext sslContext)
	: options(options), maxPayload(maxPayload), sslContext(sslContext), es(es)
	, listenPoll(nullptr), clients(nullptr)
{
    loop = es.loop;
	...
	memset(&writeStream, 0, sizeof(writeStream));
   // writeStream = {};
    ...
    if (!master) {
        ...
        uv_async_init(loop, &closeAsync, uvasynccb_handleclose);

        uv_async_init(loop, &upgradeAsync, uvasynccb_upgradehandler);
    }
...
void Server::upgrade(uv_os_sock_t fd, const char *secKey, void *ssl, const char *extensions, size_t extensionsLength)
{
	...
	UpgradeRequest ur = { fd, std::string(secKey, 24), ssl, std::string(extensions, extensionsLength) };
    upgradeQueue.push(ur);
